@extends('layouts.app', ['activePage' => 'hospital.create', 'titlePage' => __('Create Hospital')])
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        @if($hospital->id)
        <form method="post" action="{{ route('hospital.update', ['hospital' => $hospital->id]) }}" class="form-horizontal">
          @method('PUT')
          ...
          <form method="post" action="{{ route('hospital.store') }}" class="form-horizontal">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" type="text" name="name" id="name" placeholder="{{ __('Name') }}" value="{{old('name', $hospital->name)}}" required />
                      @if ($errors->has('name'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('address') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" type="address" name="address" id="address" placeholder="{{ __('Address') }}" value="{{old('address', $hospital->address)}}" required />
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" type="email" name="email" id="email" placeholder="{{ __('Email') }}" value="{{old('email', $hospital->email)}}" required />
                      @if ($errors->has('email'))
            ...
            <div class="card-footer ml-auto mr-auto ">
              <div class="row">
                <div class="col-4">
                  <a href="{{route('hospital.index')}}" class="btn btn-primary">{{ __('Cancel')}}</a>
                </div>
                <div class="col-4 text-center">
                </div>
                <div class="col-4  text-right">
                  <button type="submit" class="btn btn-primary">{{ ($hospital->id)? __('Update'): __('Create') }}</button>
                </div>
